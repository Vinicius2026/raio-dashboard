...
import { getAudios, type Material } from "@/lib/material-apoio-data";
  ...
  return (
    <header className="border-b border-white/10 bg-black/60 backdrop-blur-2xl sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center gap-2 sm:gap-4">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 transition-all group focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/60"
        >
        ...
        <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-gradient-to-br from-purple-500/90 to-purple-600 flex items-center justify-center shadow-lg shadow-purple-500/20 flex-shrink-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h1 className="text-base sm:text-xl md:text-2xl font-semibold tracking-tight text-white truncate">Material de Áudio</h1>
            <p className="text-[10px] sm:text-xs md:text-sm text-white/50 hidden sm:block">Podcasts, músicas e gravações</p>
          </div>
        </div>
      </div>
    </header>
  ...
  return (
    <div className="mb-4 sm:mb-6 md:mb-8 rounded-xl sm:rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-xl px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-4 shadow-[0_10px_40px_rgba(0,0,0,0.35)]">
      <div className="flex items-start gap-2 sm:gap-3">
        <div className="mt-0 flex h-7 w-7 sm:h-8 sm:w-8 md:h-9 md:w-9 items-center justify-center rounded-lg sm:rounded-xl bg-purple-500/15 border border-purple-500/20 text-purple-400 flex-shrink-0">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 sm:items-center">
        <div className="relative">
          ...
          <input
            ...
            className="w-full sm:w-64 pl-8 sm:pl-9 pr-2.5 sm:pr-3 py-2 sm:py-2.5 rounded-lg sm:rounded-xl bg-white/5 border border-white/10 text-xs sm:text-sm text-white placeholder:text-white/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/50"
          />
        </div>
        <div className="relative">
          ...
          <select
            ...
            className="w-full sm:w-44 pl-8 sm:pl-9 pr-7 sm:pr-8 py-2 sm:py-2.5 rounded-lg sm:rounded-xl bg-white/5 border border-white/10 text-xs sm:text-sm text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/50"
          >
  ...
  return (
    ...
    >
      <div className="col-span-12 md:col-span-6 flex items-center gap-2 sm:gap-3">
        <div className="flex-shrink-0 w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-lg bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center shadow-lg shadow-purple-500/20 group-hover:scale-105 transition-transform">
          <Mic className="w-4 h-4 sm:w-4.5 sm:h-4.5 md:w-5 md:h-5 text-white" />
        </div>
        <div className="min-w-0 flex-1">
          <a
            ...
            className="font-semibold text-white text-xs sm:text-sm truncate hover:text-purple-400 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/50 rounded block"
      ...
      <div className="col-span-12 md:col-span-2 flex items-center justify-start md:justify-end">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg bg-purple-600/90 hover:bg-purple-600 text-white text-xs sm:text-sm font-medium transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/60 w-full sm:w-auto justify-center sm:justify-start"
...
export default function AudioMaterialPage() {
  const router = useRouter();
  const materials = getAudios();
        ...
        <main className="max-w-6xl mx-auto px-4 sm:px-5 md:px-6 py-4 sm:py-6 md:py-10">
          ...
          {materials.length === 0 && (
            <div className="text-center py-20">
              <Mic className="w-16 h-16 text-white/10 mx-auto mb-4" />
              <p className="text-white/40">Nenhum material de áudio disponível no momento.</p>
            </div>
          )}
